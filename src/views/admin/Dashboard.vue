    ...
    <div v-else-if="stats" class="space-y-10">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-green-600 rounded-xl p-6 text-white flex items-center gap-6">
                ...
                <div><p class="text-sm">Total Pengguna</p><p class="text-2xl font-bold">{{ stats.statistics.total_users }}</p></div>
            </div>
            <div class="bg-blue-600 rounded-xl p-6 text-white flex items-center gap-6">
                ...
                <div><p class="text-sm">Total Tanaman</p><p class="text-2xl font-bold">{{ stats.statistics.total_plants }}</p></div>
            </div>
            <div class="bg-yellow-500 rounded-xl p-6 text-white flex items-center gap-6">
                ...
                <div><p class="text-sm">Total Pesanan</p><p class="text-2xl font-bold">{{ stats.statistics.total_orders }}</p></div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div class="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm">
              ...
              <table class="w-full text-left text-sm">
                  <thead><tr class="border-b"><th class="py-2">Order ID</th><th class="py-2">Status</th><th class="py-2">Total</th></tr></thead>
                  <tbody>
                      <tr v-for="order in stats.recent_orders" :key="order.id" class="border-b">
                          <td class="py-2 font-medium">#{{ order.id }}</td>
                          <td class="py-2"><span class="px-2 py-1 text-xs rounded-full" :class="statusClass(order.status)">{{ order.status }}</span></td>
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm">
                ...
                <ul>
                    <li v-for="plant in stats.low_stock_plants" :key="plant.id" class="flex justify-between items-center py-2 border-b">
